...
# Define a function to generate points along Cotes's spiral
def cotes_spiral_points(N, angle_increment, length_increment):
    x_values = []
    y_values = []
    x, y = 0, 0
    for _ in range(N):
        angle_rad = np.radians(angle_increment)
        length = length_increment / angle_increment
...
def generate_mosaic():
    ...
    x_values, y_values =cotes_spiral_points(N, angle_increment, length_increment)
