...
def catherine_wheel_points(N, angle_increment, length_increment):
    x_values = []
    y_values = []
    x, y = 0, 0
    for _ in range(N):
...
def generate_mosaic():
    ...
    length_increment = float(height_increment_entry.get())
    angle_increment = float(angle_increment_entry.get())

    # Generate points for the rectangular spiral pattern
    x_values, y_values = catherine_wheel_points(N, angle_increment, length_increment)
