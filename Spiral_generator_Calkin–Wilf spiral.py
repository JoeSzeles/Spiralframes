...
def calkin_wilf_sequence(n):
    sequence = [(1, 2)]  # Start with the first fraction (1/1)

    while len(sequence) < n:
        a, b = sequence[-1]  # Get the last fraction (a/b)
        
        # Generate the two child fractions by adding 1 to the numerator and denominator
        left_child = (a, a + b)
        right_child = (a + b, b)
        
        # Append the child fractions to the sequence
        sequence.append(left_child)
        sequence.append(right_child)

    return sequence[:n]  # Return the first n fractions
...
def generate_mosaic():
    ...
    # Get the value of 'turns' from the input field
    turns = float(a_entry.get())

    # Generate the Calkin–Wilf sequence
    calkin_wilf = calkin_wilf_sequence(n_points)

    # Extract the x and y coordinates from the sequence
    x_values = [fraction[0] for fraction in calkin_wilf]
    y_values = [fraction[1] for fraction in calkin_wilf]
