...
# Define a function to generate points along the triangular spiral
def triangular_spirangle_points(N, angle_increment, length_increment):
    x_values = []
    y_values = []
    x, y = 0, 0
    for _ in range(N):
        # Calculate the end coordinates of the three line segments forming an equilateral triangle
        for _ in range(3):
            x_end = x + length_increment
            y_end = y
            ...
            # Rotate by 120 degrees to form the next side of the triangle
            x, y = (
                np.cos(np.radians(120)) * x - np.sin(np.radians(120)) * y,
                np.sin(np.radians(120)) * x + np.cos(np.radians(120)) * y,
            )
...
def generate_mosaic():
    ...
    x_values, y_values = triangular_spirangle_points(N, angle_increment, length_increment)
